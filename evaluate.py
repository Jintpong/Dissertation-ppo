...
seeds = [1, 2, 3]
timestep_values = [500_000, 1000_000, 200_0000]
...
# Ensures reproducible results by setting the seed 
def set_seed(seed):
    np.random.seed(seed)
    random.seed(seed)
    torch.manual_seed(seed)

def make_evaluation_env(seed):
    def _init():
        env = Monitor(Wheat(mode = 'eval', year1=1982, year2=2018 ))
        set_seed(seed)
        return env 
    evaluate_env = DummyVecEnv([_init])
    return evaluate_env
...
def load_ppo_model_timestep(seed, timestep):
    set_seed(seed)
    eval_env = make_evaluation_env(seed)
...
path = get_filepath('/Users/jintpongchababnapa/Documents/Dissertation/agent/weather_data/champion_climate.txt')
wdf = prepare_weather(path)
sim_start = "1982/01/01"
sim_end = "2018/12/31"
soil = Soil('Loam')
crop_obj = Crop('Wheat', planting_date='01/01')
...
iririgation_methods = [rainfed, threshold4_irrigate, interval_7]
labels = ['Rainfed', 'Thresholds', 'Interval']
outputs = []

for i, irr_mngt in enumerate(iririgation_methods):
